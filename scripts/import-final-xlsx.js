...
const XLSX_PATH = './vendor_catalogue_final.xlsx';
const API_URL = 'http://localhost:3001/api/products/import';
const SERVICE_FEE = 5;
...
function detectColumns(headers) {
  ...
  const categoryCol = headers.find((_, i) => lower[i].includes('category') && !lower[i].includes('sub')) ?? headers.find((_, i) => lower[i].includes('category'));
  const subCategoryCol = headers.find((_, i) => lower[i].includes('sub') && lower[i].includes('cat'));
  const priceCol = headers.find((_, i) => {
    const h = lower[i];
    return (h.includes('buying') || h.includes('cost') || h.includes('amount') || (h.includes('price') && !h.includes('selling'))) && !h.includes('nomad') && !h.includes('service') && !h.includes('fee');
  });
  const sellingPriceCol = headers.find((_, i) => {
    const h = lower[i];
    return h.includes('selling') && h.includes('price');
  });
  const skuCol = headers.find((_, i) => lower[i] === 'id' || lower[i] === 'sku') ?? headers.find((_, i) => lower[i].includes('sku') || lower[i].includes('barcode') || lower[i].includes('bar_code') || lower[i].includes('bar code') || lower[i].includes('item id') || lower[i].includes('product id'));
  const unitCol = headers.find((_, i) => lower[i].includes('unit') || lower[i].includes('pack') || lower[i].includes('size') || lower[i].includes('uom'));
  const imageCol = headers.find((_, i) => lower[i].includes('image') || lower[i].includes('img') || lower[i].includes('photo') || lower[i].includes('picture') || lower[i] === 'url');
  ...
  return { nameCol, categoryCol, subCategoryCol, priceCol, sellingPriceCol, skuCol, unitCol, imageCol, vatCol, stockCol, l1Name, l1Qty, l1Cost, l1Sell, l1Barcode, l2Name, l2QtyInL1, l2Cost, l2Sell, l2Barcode };
}

function extractMultiplier(name) {
  const patterns = [/(\d+)\s*[*xX]\s*\d+/, /(\d+)\s*pcs/i, /pack\s*of\s*(\d+)/i, /case\s*of\s*(\d+)/i, /(\d+)\s*units/i];
...
async function main() {
  ...
  rows.forEach((row, i) => {
    ...
    if (!name || name === 'FILL IN — standard market name') return;
    ...
    const sellingPrice = rawSellingPrice > 0 ? rawSellingPrice : costPrice > 0 ? Number((costPrice * (1 + SERVICE_FEE / 100)).toFixed(2)) : 0;
    ...
    const baseSku = (rawSku && rawSku !== 'DO NOT EDIT — system key' && rawSku !== 'FILL IN if known')
      ? rawSku.replace(/\s+/g, '-')
    ...
    if (l1NameVal && l1QtyVal > 0) {
      ...
      packagingTiers.push({ name: unit, level: 0, quantityInBase: 1, costPrice: costPrice, sellingPriceOverride: sellingPrice, barcode: rawSku || null, isBaseUnit: true });
      packagingTiers.push({
        name: l1NameVal,
        level: 1,
  ...
  console.log(`Parsed ${products.length} products. Sending to API...`);

  const res = await fetch(API_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ products, replace: true })
  });
